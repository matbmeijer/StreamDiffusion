...
class Terrain(object):
    def __init__(self):
        ...
        self.window.setGeometry(0, 200, 1920, 1080)
    ...
    def mesh(self, offset=0, height=2.5, wf_data=None):
        if wf_data is not None:
            ...
            wf_data = np.array(wf_data, dtype=np.int64)[::2] + 128
        ...
        verts = np.array(
            [
                [x, y, wf_data[xid][yid] * self.noise.noise2(x=xid / 5 + offset, y=yid / 5 + offset) * 0.25]
    ...
    def update(self):
        ...
        wf_data = np.frombuffer(wf_data, dtype=np.int16)  # Convert byte stream to numpy array of int16
